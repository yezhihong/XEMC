inline void gCal_DIS(double kE0,double kEp,double kTheta, XEM_TGT* kTarget,double kPmax, int kInnt, int kInnp,XEM_SIG* sig)
{
	...
	*/
	...
	Double_t Mass_A= Mass_A_1 + PROTON_MASS - esep;
	...
	for( int ip=1;ip<=kInnp;ip++){
		...
		for(int iu=1;iu<=kInnt;iu++){
			...
			nu_p = (Ei*nu - p*u*Qv)/PROTON_MASS;
			WpMin2 = pow((PROTON_MASS+0.135),2);
			...
			if(radical> WpMin2){
				...
				if(!(w11==0)&&!(w22==0)){
					...
					arg21 = pow((1.0+p*u*Qsq/PROTON_MASS/nu_p/Qv),2)
						*nu_p*nu_p/nu/nu;
